...
export default function FullScreenbanner({}) {
  ...
  const [topmiddleBanner, settopmiddleBanner] = useState();
  const [bannerData, setbannerData] = useState<any>();

  useEffect(() => {
    topMiddleBanner();
  }, []);
  const topMiddleBanner = async () => {
    try {
      const response = await Api.get(
        `${Endpoints.getBanners}?page=home&pageReference=top-middle`,
        {
          headers: {},
        }
      );
  ...
  const handleBannerClick = (linkTypes: any, linkValues: any) => {
    ...
    if (linkTypes == "category" || linkTypes == "brand") {
      ...
      // push(`/products/products-listing?id=${linkValues}&linkType=${linkTypes}`);
  ...
  return (
    <div>
      ...
      >
        <img
          src={`${Api.defaults.baseURL}${topmiddleBanner}`}
