...
export default function Kitchenbanner({}) {
  ...
  const [middletop, setmiddletop] = useState();
  const [bannerData, setbannerData] = useState<any>();

  useEffect(() => {
    MiddleTopBanner();
  }, []);
  const MiddleTopBanner = async () => {
    try {
      const response = await Api.get(
        `${Endpoints.getBanners}?page=home&pageReference=middle-top`,
        {
          headers: {},
        }
      );
  ...
  return (
    <div>
      ...
      >
        <img
          src={`${Api.defaults.baseURL}${middletop}`}
